...
def get_normalized_pixel_grid_kilo(opt, H, W):
    y_range = ((torch.arange(H,dtype=torch.float32,device=opt.device))/H*2-1)*(H/max(H,W))
    x_range = ((torch.arange(W,dtype=torch.float32,device=opt.device))/W*2-1)*(W/max(H,W))
...
def get_normalized_pixel_grid(opt):
    y_range = ((torch.arange(opt.H,dtype=torch.float32,device=opt.device))/opt.H*2-1)*(opt.H/max(opt.H,opt.W))
    x_range = ((torch.arange(opt.W,dtype=torch.float32,device=opt.device))/opt.W*2-1)*(opt.W/max(opt.H,opt.W))
    Y,X = torch.meshgrid(y_range,x_range) # [H,W]
    xy_grid = torch.stack([X,Y],dim=-1).view(-1,2) # [HW,2]
    xy_grid = xy_grid.repeat(opt.batch_size,1,1) # [B,HW,2]
    return xy_grid

def get_normalized_pixel_grid_equal(opt):
    pixel_coords = np.stack(np.mgrid[:opt.H, :opt.W], axis=-1)[None, ...].astype(np.float32)
    pixel_coords[0, :, :, 0] = pixel_coords[0, :, :, 0] / (opt.H - 1)
    pixel_coords[0, :, :, 1] = pixel_coords[0, :, :, 1] / (opt.W - 1)
    pixel_coords -= 0.5
    pixel_coords *= 2.
    pixel_coords = torch.Tensor(pixel_coords).view(-1, 2).unsqueeze(0)
    return pixel_coords.cuda()


def get_normalized_pixel_grid_zero_to_one(opt):
    y_range = ((torch.arange(opt.H,dtype=torch.float32,device=opt.device))/opt.H)*(opt.H/max(opt.H,opt.W))
    x_range = ((torch.arange(opt.W,dtype=torch.float32,device=opt.device))/opt.W)*(opt.W/max(opt.H,opt.W))
    Y,X = torch.meshgrid(y_range,x_range) # [H,W]
    xy_grid = torch.stack([X,Y],dim=-1).view(-1,2) # [HW,2]
    xy_grid = xy_grid.repeat(opt.batch_size,1,1) # [B,HW,2]
    return xy_grid
