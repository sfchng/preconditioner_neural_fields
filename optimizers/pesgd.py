...
class PESGD(Optimizer):
    ...
    def __init__(self, params,  lr=10, betas=(0.9, 0.999), lr_warmup=0.99, eps=1e-4,
                 weight_decay=0, update_d_every=100, d_warmup=50, outpath=None):
        if not 0. <= lr:
        ...
        defaults = dict(  outpath=outpath, lr=lr, betas=betas, lr_warmup=lr_warmup, eps=eps,
                        weight_decay=weight_decay, update_d_every=update_d_every, d_warmup=d_warmup)
        super(PESGD, self).__init__(params, defaults)
        self.update_d_every = params[0]['update_d_every']
        self.d_warmup = params[0]['d_warmup']
        self.steps = 0
            ...
            with torch.enable_grad():
                for group in self.param_groups:
                    for p in group['params']:
                        ...
                        #vs.append(torch.randint_like(p.grad, 2) )
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if hvps_iter is not None:
                    ...
                    #grad2 = grad.mul(grad.conj())
                    Qh = state['precond_a'] * hvp + state['precond_b'] * torch.flip(grad, [0])

                    #exp_avg_d.mul_(beta2).addcmul_(hvp, hvp.conj(), value=1-beta2)
                    exp_avg_d.mul_(beta2)
                    torch.maximum(exp_avg_d, Qh.square_(), out=exp_avg_d)
