            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if len(state) == 0:
                        ...
                        ## Preconditioner ##
                        state['precond_a'] = torch.ones_like(p, memory_format=torch.preserve_format)
                        state['precond_b'] = torch.ones_like(p, memory_format=torch.preserve_format)*0.5
                    ...
                    precond_as.append(state['precond_a'])
                    precond_bs.append(state['precond_b'])
...
def adam(params: List[Tensor],
         ...
         precond_as: List[Tensor],
         precond_bs: List[Tensor],
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        precond_a = precond_as[i]
        precond_b = precond_bs[i]
        ...
        Qh = precond_a * grad + precond_b * torch.flip(grad, [0])

        ## flip ##
        # aflip, bflip = torch.flip(precond_a, [0]), torch.flip(precond_b, [0])
        # invQtv = (aflip*grad - bflip*torch.flip(grad, [0]))/(precond_a*aflip - precond_b*bflip)
        # nablaA = Qh*Qh - invQtv*invQtv
        # nablaB = Qh*torch.flip(Qh, [0]) - invQtv*torch.flip(invQtv, [0])
        # q, r = divmod(len(nablaB), 2)
        # if r == 1:
        #     nablaB[q] = 0
        # mu = 0.01/(torch.maximum(torch.max(torch.abs(nablaA)), torch.max(torch.abs(nablaB))) + 1.2e-38)
        # precond_a.sub_(mu*(nablaA*precond_a + nablaB*bflip))
        # precond_b.sub_(mu*(nablaA*precond_b + nablaB*aflip))

        # Decay the first and second moment running average coefficient
        exp_avg.mul_(beta1).add_(grad, alpha=1 - beta1)
        exp_avg_sq.mul_(beta2).addcmul_(Qh, Qh.conj(), value=1 - beta2)
