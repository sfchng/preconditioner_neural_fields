...
class Model():
    ...
    def train_iteration(self,opt,var,loader):
        ...
        var = self.graph.forward(opt,mode="train")
    ...
    @torch.no_grad()
    def save_checkpoint_kilo(self,opt,network_ind, ep=0,it=0, latest=False):
        util.save_checkpoint_kilo(opt,self,network_ind,ep=ep,it=it,latest=latest)
        if not latest:
            log.info("checkpoint saved: ({0}) {1}, epoch {2} (iteration {3})".format(opt.group,opt.name,ep,it))

    @torch.no_grad()
    def save_checkpoint_kilo_v2(self,opt,network_ind, grid_network_ind, ep=0,it=0, latest=False):
        util.save_checkpoint_kilo_v2(opt,self,network_ind, grid_network_ind,ep=ep,it=it,latest=latest)
        if not latest:
            log.info("checkpoint saved: ({0}) {1}, epoch {2} (iteration {3})".format(opt.group,opt.name,ep,it))
